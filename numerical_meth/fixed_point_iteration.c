 ...
 */
double phi(double x) { return 2 - x * x; }

double g(double x) { return x * x - 5; }

double phi_2(double x) { return cbrt(5 * x); }
...
int main() {
    printf("Approximation for 1a is %lf\n", approximator(f, phi));
    printf("Approximation for 1b is %lf", approximator(g, phi_2));
}
